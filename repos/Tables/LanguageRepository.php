...
class LanguageRepository {
	...
	function getLanguagesForProject($project) {
		if (isset($project) && $project > 0) {
			$sql = "SELECT languages.id, languages.language from project_languages JOIN languages ON project_languages.language = languages.id WHERE project_languages.project = ?";
			$statement = $this->conn->prepare($sql);
